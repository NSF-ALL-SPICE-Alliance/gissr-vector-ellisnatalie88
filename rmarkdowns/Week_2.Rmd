...
```{r}
  df_layers_cultural
str(df_layers_cultural)
  df_layers_physical
str(df_layers_physical)
```
...
ggplot() +
  geom_sf(data = us_states)+
  geom_sf(data = pacific_groups)+
  geom_sf_text(data = us_states, aes(label = admin))+
  coord_sf(crs = "+proj=ortho +lat_0=0 +lon_0=-160 ") +
  coord_sf(xlim = c(-158.28058, -157.64863), ylim = c(21.25482, 21.71201), expand = FALSE)+
  ggtitle("Oahu - Hawaii", subtitle = "Showing Pacific Island Groupings")+
  theme_stata()
```
...
```{r}
#unable to produce the correct map
ggplot() +
  geom_sf(data = us_states)+ 
  geom_sf(data = lakes50, fill = "darkblue", color = "black") +
  geom_sf(data = rivers50, color = "yellow", size = 0.15) +
  geom_sf(data = ocean50, color = "red", size = 0.15) +
  geom_sf_text(data = us_states, aes(label = name), size = 5, fontface = "bold", nudge_y = states$nudge_y) +
  coord_sf(xlim = c(-158.28058, -157.64863), ylim = c(21.25482, 21.71201), expand = FALSE) + 
  ggtitle("Oahu - Hawaii", subtitle = "Showing Lake & River & Ocean")+
  theme_stata()
```
...
```{r}
ggplot() +
    geom_sf(data = reefs, fill = "lightgray") +
...
# Download the protected areas layer
reefs <- ne_download(
  type = "reefs", 
  category = "physical", 
  returnclass = "sf")
