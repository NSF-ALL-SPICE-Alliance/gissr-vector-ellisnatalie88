...
```{r}
# this website is a good reference to find out the available tags https://wiki.openstreetmap.org/wiki/Map_features 

available_tags(feature = "healthcare:speciality")  
#unable to show result 

available_tags(feature = "healthcare:counselling")
#unabl to show result 

available_tags(feature = "traffic_calming")

available_tags(feature = "electric_bicycle")
...
# meaning of th code 
# healthcare <- get the bounding box for Oahu Healthcare *and then*
# prepare an OSM query *and then*
# specify the types of amenity to include hospital *and then*
...
ggplot() +
  ...
  geom_sf_text(data = healthcare$osm_polygons, aes(label = name), size = 2, hjust = 0) + 
  coord_sf(xlim = c(-158.28058, -157.64863), 
           ylim = c(21.25482,  21.71201),
           expand = FALSE)
```
...
Oahu_small_streets <- Oahubb %>%
  opq() %>%
  add_osm_feature(key = "highway", value = c("residential", "living_street", "unclassified", "service", "footway")) %>%
  osmdata_sf()

Oahu_coastline <- Oahubb %>%
  opq() %>%
  add_osm_feature(key = "natural", value = "coastline") %>%
  osmdata_sf()
```
...
ggplot() +
  ...
  geom_sf(data = Oahu_small_streets$osm_lines, inherit.aes = FALSE, color = "#a6a6a6", size = .2, alpha = .8) +
  geom_sf(data = Oahu_coastline$osm_lines, inherit.aes = FALSE, color = "black", size = .8, alpha = .5) +
  geom_sf(data = healthcare$osm_polygons, inherit.aes = FALSE, color = "red", fill = "red", alpha = 0.9, size = 1) +
  coord_sf(xlim = c(-158.28058,-157.64863), ylim = c(21.25482, 21.71201), expand = TRUE) +
  geom_sf_text(data = healthcare$osm_polygons, aes(label = name), size = 1.5, hjust = 0) +
  theme_solarized_2() +
  labs(
    title = "Hospitals in Oahu - Hawaii",
    x = "Longitude",
    y = "Latitude"
  )
...
leaflet() %>%  
  addTiles() %>%
  addPolygons(data = healthcare$osm_polygons,
              label = healthcare$osm_polygons$name)
```
